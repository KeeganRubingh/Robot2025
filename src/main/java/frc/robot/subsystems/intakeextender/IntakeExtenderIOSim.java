...
public class IntakeExtenderIOSim implements IntakeExtenderIO {
  ...
  public IntakeExtenderIOSim(int motorId) {
    sim =  new FlywheelSim(LinearSystemId.createFlywheelSystem(DCMotor.getKrakenX60Foc(1), 0.28616, 1),DCMotor.getKrakenX60Foc(1),new double[] {0.001});
  }
  ...
  @Override
  public void updateInputs(IntakeExtenderInputs input) {
    // updinputs
    input.Angle.mut_replace(Degrees.convertFrom(sim.getOutput(0), Radians), Degrees);
    input.IntakeExtenderAngularVelocity.mut_replace(
